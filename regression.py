# 5. Data Modeling > Lab: Regression > Lab 9
import pandas as pd
import numpy as np
import matplotlib
...
Xi = pd.read_csv("Datasets/College.csv")
...
X = Xi[['Accept','Enroll','F.Undergrad','Room.Board']]
'''
X.isnull().any()
Out[10]: 
Accept         False
...
'''
score1
Out[31]: 0.0062114538852888757
'''
# INFO: We'll take it from here, buddy:
drawLine(model, X1_test, y1_test, "Accept(Room&Board)", score1)
...
'''
score2
Out[34]: 0.85782048673561562
'''

drawLine(model, X2_test, y2_test, "Accept(Enroll)", score2)
...
score3 = model.score(X3_test, y3_test)
'''
score3
Out[37]: 0.77799179737540058
'''

drawLine(model, X3_test, y3_test, "Accept(F.Undergrad)", score3)
...
y4 = X[['Accept']]
X4_train, X4_test, y4_train, y4_test = train_test_split(X4, y4, test_size=0.30, random_state=7)
model.fit(X4_train, y4_train)
score4 = model.score(X4_test, y4_test)
'''
score4
Out[39]: 0.8764371774214047
'''

drawPlane(model, X4_test, y4_test, "Accept(Room&Board,Enroll)", score4)


#
# INFO: That concludes this assignment 5. Data Modeling > Lab: Regression > Lab 9
